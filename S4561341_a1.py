...
def floorHeight(floor_x, floor_z):
    ...
    plt.imshow(foundation_heightmap, cmap='hot', interpolation='nearest')
    plt.show()
...
def getRandomPlank():
    ids = ["minecraft:warped_planks", "minecraft:acacia_planks", 
           "minecraft:oak_planks", "minecraft:birch_planks", "minecraft:crimson_planks"]
    return ids[np.random.randint(0, len(ids))]
...
def getRandomGlassBlock():
    ids = ["minecraft:glass", "minecraft:lime_stained_glass", "minecraft:pink_stained_glass",
           "minecraft:blue_stained_glass", "minecraft:orange_stained_glass", "minecraft:yellow_stained_glass",
           "minecraft:magenta_stained_glass", "minecraft:white_stained_glass"]
    return ids[np.random.randint(0, len(ids))]
...
def buildRoof(start, end):
    ...
    roof_start = np.random.randint(-1, 1)

    for i in range(roof_start, ((end[coord_idx[idx]]-start[coord_idx[idx]])//2)+1): 
        if idx==1:
...
def buildGarden(start, end, direction, footpath_pos):
    ...
    if direction in ["north", "south"]:
        ...
        geo.placeCuboid(editor,
                        (start[0]-1, start[1]-1, start[2]+(1 if direction=="north" else -1)),
                        (end[0]+1, end[1]-1, end[2]+ (garden_depth if direction=="north" else -garden_depth)),
                        Block(getRandomPlank()))
        ...
        geo.placeCuboid(editor,
                        ...
                        Block("minecraft:grass_block"))
        geo.placeCuboid(editor,
                        (start[0]+(-1 if direction=="east" else 1), start[1]-1, start[2]-1),
                        (end[0]+ (-garden_depth if direction=="east" else garden_depth), end[1]-1, end[2]+1),
                        Block(getRandomPlank()))
        ...
        for i in range(start[2], end[2]+1):
            if i!= footpath_pos: #place flower pots
                ...
                geo.placeCuboid(editor,
                            ...
                            Block("minecraft:cobblestone"))
    buildSupportBeam((footpath_end[0], footpath_end[1]-1, footpath_end[2]))
...
def buildRectWindow(start, end, direction, building_height):  
    if direction in ["east", "west"]:
        window_start = (start[0], 
                        start[1]+2, 
                        np.random.randint(start[2],end[2]-1))

        window_end = (end[0], 
                      np.random.randint(start[1]+2,end[1]+building_height), 
                      np.random.randint(window_start[2]+1, end[2]+1))

    else:
        window_start = (np.random.randint(start[0],end[0]-1), 
                        start[1]+2, 
                        start[2])

        window_end = (np.random.randint(window_start[0]+1, end[0]+1), 
                      np.random.randint(start[1]+2,end[1]+building_height), 
                      end[2])  
...
def buildBookShelf(start, end, direction, height_limit):
    ...
    if direction in ["east", "west"]:
        ...
        shelf_end = (end[0], 
                      ...
                      np.random.randint(shelf_start[2], end[2]+1))

    else:
        shelf_start = (np.random.randint(start[0],end[0]+1), 
                        start[1], 
                        start[2])

        shelf_end = (np.random.randint(shelf_start[0], end[0]+1), 
                      np.random.randint(start[1], end[1]+height+1), 
                      end[2])  
        
    geo.placeCuboid(editor, shelf_start, shelf_end, Block("minecraft:bookshelf"))
...
def buildSingleFloorAndRoof(start: tuple):
    ...
    interior_corners=[[(corner1[0]+1,corner1[1]+1,corner1[2]+1),(corner2[0]-1,corner2[1]+1,corner2[2]+1),"south"],
                      ...
                      [(corner3[0]+1,corner3[1]+1,corner3[2]-1),(corner4[0]-1,corner4[1]+1,corner4[2]-1),"north"]]

    for corner in corners:
        buildSupportBeam(corner)

    ##Main shape of the house
    ...
    ##Clear the room
    geo.placeCuboid(editor,
                    interior_corners[0][0],
                    (interior_corners[3][1][0],interior_corners[3][1][1]+building_ydim-1,interior_corners[3][1][2]),
                    Block("minecraft:air"))
    
    ##Build glass ceiling
    geo.placeCuboid(editor,
                    (interior_corners[0][0][0],interior_corners[0][0][1]+building_ydim,interior_corners[0][0][2]),
                    (interior_corners[3][1][0],interior_corners[3][1][1]+building_ydim-1,interior_corners[3][1][2]),
                    Block(getRandomGlassBlock()))
    ...
    ##Build bookshelf
    getOppositeCardinal(first_door_wall[2])
    shelf_side = random.choice([side for side in interior_corners if side[2]== getOppositeCardinal(first_door_wall[2])])
    buildBookShelf(shelf_side[0], shelf_side[1], shelf_side, building_ydim)

    ##Build window
    window_side = random.choice([side for side in wall_corners_pairs if side[2] not in [shelf_side[2], first_door_wall[2]]])
...
#         buildSingleFloorAndRoof((STARTX+x, STARTY, STARTZ+z))
